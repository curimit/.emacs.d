;;; company-stylus.el --- company-mode completion back-end for stylus-mode  -*- lexical-binding: t -*-
...
(defconst company-stylus-property-alist
...
(defconst company-stylus-value-classes
...
(defconst company-stylus-html-tags
...
(defconst company-stylus-pseudo-classes
  '("active" "after" "before" "first" "first-child" "first-letter" "first-line"
    "focus" "hover" "lang" "left" "link" "right" "visited")
  "Identifiers for CSS pseudo-elements and pseudo-classes.")

(defconst company-stylus-property-cache (make-hash-table :size 115 :test 'equal))

(defun company-stylus-property-values (attribute)
  "Access the `company-stylus-property-alist' cached and flattened."
  (or (gethash attribute company-stylus-property-cache)
      (let (results)
        (dolist (value (cdr (assoc attribute company-stylus-property-alist)))
          (if (symbolp value)
              (dolist (child (or (cdr (assoc value company-stylus-value-classes))
                                 (company-stylus-property-values
                                  (symbol-name value))))
                (push child results))
            (push value results)))
        ...
        (puthash attribute
                 (if (fboundp 'delete-consecutive-dups)
                   ...
                   (delete-dups results))
                 company-stylus-property-cache)
        results)))

;;; bracket detection

(defconst company-stylus-braces-syntax-table
...
(defun company-stylus-inside-braces-p ()
  "Return non-nil, if point is within matched { and }."
  't)

;;; tags
(defconst company-stylus-tag-regexp
  (concat "\\(?:\\`\\|}\\)[[:space:]]*"
          ;; multiple
          "\\(?:"
          ;; previous tags:
...
;;; pseudo id
(defconst company-stylus-pseudo-regexp
  (concat "\\(?:\\`\\|}\\)[[:space:]]*"
          ;; multiple
          "\\(?:"
          ;; previous tags:
...
(defun company-stylus-grab-property ()
  "Return the CSS property before point, if any.
Returns \"\" if no property found, but feasible at this position."
  (company-grab-symbol))

;;; values
(defconst company-stylus-property-value-regexp
  "\\_<\\([[:alpha:]-]+\\):?[ ]\\="
  "A regular expression matching CSS tags.")

;;;###autoload
(defun company-stylus (command &optional arg &rest ignored)
  "`company-mode' completion back-end for `css-mode'."
  (interactive (list 'interactive))

  (cl-case command
    (interactive (company-begin-backend 'company-stylus))
    (prefix (and (derived-mode-p 'stylus-mode)
                 (or (company-grab company-stylus-tag-regexp 1)
                     (company-grab company-stylus-pseudo-regexp 1)
                     (company-grab company-stylus-property-value-regexp 2)
                     (company-stylus-grab-property))))
    (candidates
     (cond
      ((company-grab company-stylus-tag-regexp 1)
       (all-completions arg company-stylus-html-tags))
      ((company-grab company-stylus-pseudo-regexp 1)
       (all-completions arg company-stylus-pseudo-classes))
      ((company-grab company-stylus-property-value-regexp 2)
       (all-completions arg
                        (company-stylus-property-values
                         (company-grab company-stylus-property-value-regexp 1))))
      ((company-stylus-grab-property)
       (all-completions arg company-stylus-property-alist))))
    (sorted t)))

(provide 'company-stylus)
;;; company-stylus.el ends here
