...
(defvar htmlize-basic-character-table
  ...
  (let ((table (make-vector 128 ?\0)))
    ...
    (setf
     ...
     (aref table ?\t) "\t"
     ;; Escape &, <, and >.
     (aref table ?&) "&amp class="comment">;"
     (aref table ?<) "&lt class="comment">;"
     (aref table ?>) "&gt class="comment">;"
...
(defun htmlize-make-hyperlinks ()
  ...
  (while (re-search-forward
	  "&lt class="comment">;\\(\\(mailto:\\)?\\([-=+_.a-zA-Z0-9]+@[-_.a-zA-Z0-9]+\\)\\)&gt;"
	  nil t)
    (let ((address (match-string 3))
	  (link-text (match-string 1)))
      (delete-region (match-beginning 0) (match-end 0))
      (insert "&lt class="comment">;<a href=\"mailto: class="string">"
	      (htmlize-despam-address address)
	      "\">"
	      (htmlize-despam-address link-text)
	      "</a>&gt class="comment">;")))
  (goto-char (point-min))
  (while (re-search-forward "&lt class="comment">;\\(\\(URL:\\)?\\([a-zA-Z]+://[^;]+\\)\\)&gt;"
			    nil t)
    (let ((url (match-string 3))
	  (link-text (match-string 1)))
      (delete-region (match-beginning 0) (match-end 0))
      (insert "&lt class="comment">;<a href=\"" url "\">" link-text "</a>&gt;"))))
...
(defun htmlize-defang-local-variables ()
  ...
  (goto-char (point-min))
  (while (search-forward "Local Variables: class="string">" nil t)
